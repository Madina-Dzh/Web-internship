...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sdate'])) {
    ...
    if (!isValidDateString($newDateStr)) {
        ...
        header("Location: individual-details.php?id=$id&message=Ошибка:%20неверный%20формат%20даты&type=error");
        exit;
    } else {
        // Экранируем для SQL
        ...
        if ($mysql->query($updateQuery)) {
            ...
            $mysql->query("INSERT INTO `user_actions`(`action_text`) VALUES ('дата обновлена для индивидуального договора № $number')");
            header("Location: individual-details.php?id=$id&message=Дата%20успешно%20обновлена&type=success");
            exit;
        } else {
            // Ошибка обновления — перенаправляем с сообщением ошибки
            header("Location: individual-details.php?id=$id&message=Ошибка%20при%20обновлении:%20" . urlencode($mysql->error) . "&type=error");
            exit;
        }
    }
}
